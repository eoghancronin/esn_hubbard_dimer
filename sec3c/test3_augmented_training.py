...
def train_esn(X, Y, params, warm_up, verbose=0):
    ...
    esn = ESN(reservoir=reservoir, readout=readout, workers=22)
...
def objective(trial, X_train, Y_train, warm_up, X_test, Y_test):
    """
    ...
    for i, channel_name in enumerate(channel_names):
        ...
        connectivity = trial.suggest_float(f'input_connectivity_{channel_name}', 2E-3, 0.3, log=False)
    ...
    results = evaluate_esn(esn, X_test, Y_test, warm_up)
...
def train_optimal_esn_with_test(study_name, test_index, n_trials=0, load_if_exists=True):
    ...
    file_name_validation = 'test'+f'{test_index}'+'_validation_data.json'
    ...
    validation_data = load_data(file_name_validation, test_index)
    ...
    if n_trials > 0:
        ...
        study.optimize(
            lambda trial: objective(trial, X_train, Y_train, warm_up, X_validation, Y_validation), 
            n_trials=n_trials
        )
