...
def generate_test2_data():
    ...
    delta_v0_train = -1.3 + 0.0*np.linspace(0, 1, num_sys_train)
    amplitude_train = -1*delta_v0_train
    dt = 0.2
    tmax = 2000
    t_on_train = (tmax/2)*np.ones(num_sys_train)
    ...
    file_name_train = 'test2_train_data.json'
    ...
    delta_v0_test = -1.3 + 0.0*np.linspace(0, 1, num_sys_test)

    amplitude_test = -1*delta_v0_test
    t_on_test = (tmax/2)*np.ones(num_sys_test)
    ...
    file_name_test = 'test2_test_data.json'
